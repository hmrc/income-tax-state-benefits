...
import connectors.responses.DeleteStateBenefitDetailOverrideResponse.deleteStateBenefitDetailOverrideResponseReads
import play.api.http.Status._
import play.api.libs.json.{JsValue, Json}
import support.UnitTest
import uk.gov.hmrc.http.HttpResponse

class DeleteStateBenefitDetailOverrideResponseSpec extends UnitTest {
  ...
  private val underTest = deleteStateBenefitDetailOverrideResponseReads

  "deleteStateBenefitDetailOverrideResponseReads" should {
    "convert JsValue to DeleteStateBenefitDetailOverrideResponse" when {
      "status is NO_CONTENT and any jsValue" in {
        val httpResponse = HttpResponse.apply(NO_CONTENT, "", anyHeaders)

        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe DeleteStateBenefitDetailOverrideResponse(
          httpResponse,
          Right(())
        )
      }
