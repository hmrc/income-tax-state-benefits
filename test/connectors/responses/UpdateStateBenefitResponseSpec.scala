...
class UpdateStateBenefitResponseSpec extends UnitTest {
  ...
  private val underTest = UpdateStateBenefitResponse.updateStateBenefitResponseReads

  ".updateStateBenefitResponseReads" should {
    "convert JsValue to UpdateStateBenefitResponse" when {
      "status is CREATED and any jsValue" in {
        val httpResponse = HttpResponse.apply(CREATED, "", anyHeaders)

        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe UpdateStateBenefitResponse(httpResponse, Right(()))
      }

      "status is BAD_REQUEST and jsValue" in {
        val httpResponse = HttpResponse.apply(BAD_REQUEST, singleErrorBodyJson, anyHeaders)

        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe UpdateStateBenefitResponse(
          httpResponse,
