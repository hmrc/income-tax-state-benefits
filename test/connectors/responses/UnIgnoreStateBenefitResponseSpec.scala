...
import connectors.responses.UnIgnoreStateBenefitResponse.unIgnoreStateBenefitResponseReads
import play.api.http.Status._
import play.api.libs.json.{JsValue, Json}
import support.UnitTest
import uk.gov.hmrc.http.HttpResponse

class UnIgnoreStateBenefitResponseSpec extends UnitTest {
  ...
  private val underTest = unIgnoreStateBenefitResponseReads

  "unIgnoreStateBenefitResponseReads" should {
    "convert JsValue to UnIgnoreStateBenefitResponse" when {
      "status is NO_CONTENT and any jsValue" in {
        val httpResponse = HttpResponse.apply(NO_CONTENT, "", anyHeaders)

        underTest.read(anyMethod, anyUrl, httpResponse) shouldBe UnIgnoreStateBenefitResponse(
          httpResponse,
          Right(())
        )
      }
