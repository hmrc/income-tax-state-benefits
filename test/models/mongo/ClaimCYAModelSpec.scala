...
class ClaimCYAModelSpec extends UnitTest
  ...
  "EncryptedClaimCYAModel.decrypted" should {
    "return ClaimCYAModel" in {
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedBenefitId.value, encryptedBenefitId.nonce, associatedText).returning(aClaimCYAModel.benefitId.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedStartDate.value, encryptedStartDate.nonce, associatedText).returning(aClaimCYAModel.startDate.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedEndDateQuestion.value, encryptedEndDateQuestion.nonce, associatedText).returning(aClaimCYAModel.endDateQuestion.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedEndDate.value, encryptedEndDate.nonce, associatedText).returning(aClaimCYAModel.endDate.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedDateIgnored.value, encryptedDateIgnored.nonce, associatedText).returning(aClaimCYAModel.dateIgnored.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedSubmittedOn.value, encryptedSubmittedOn.nonce, associatedText).returning(aClaimCYAModel.submittedOn.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedAmount.value, encryptedAmount.nonce, associatedText).returning(aClaimCYAModel.amount.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedTaxPaidQuestion.value, encryptedTaxPaidQuestion.nonce, associatedText).returning(aClaimCYAModel.taxPaidQuestion.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedTaxPaid.value, encryptedTaxPaid.nonce, associatedText).returning(aClaimCYAModel.taxPaid.get.toString)
      (aesGcmAdCrypto.decrypt(_: String, _: String)(_: String))
        .expects(encryptedIsHmrcData.value, encryptedIsHmrcData.nonce, associatedText).returning(aClaimCYAModel.isHmrcData.toString)
